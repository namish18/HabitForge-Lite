...
export async function getProfile(userId) {
  ...
  const { content } = await ensureFile(PATHS.profile, {});
  return content || {};
}

export async function saveProfile(profile, userId) {
  const PATHS = getPaths(userId);
  const { sha } = await readFile(PATHS.profile, {});
  await writeFile(PATHS.profile, profile, 'Update profile', sha);
}
...
/**
 * Returns a map of all available log entries keyed by date string.
 * { "2026-06-28": encryptedPayload, ... }
 * Used by the backup/export endpoint.
 */
export async function getAllLogs(userId, daysBack = 90) {
  const dates = await getLogDates(userId, daysBack);
  const entries = await Promise.all(
    dates.map(async (date) => {
      const content = await getDailyLog(date, userId);
      return [date, content];
    })
  );
