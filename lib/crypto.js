...
// Restore from localStorage on module load (shared across all tabs; survives
// page refreshes and new tabs; cleared only on explicit logout).
let _sessionPassword = (
  typeof window !== 'undefined'
    ? (localStorage.getItem(_SK) ?? null)
    : null
);

/**
 * Store the user's password in memory and in localStorage so that
 ...
 *
 * localStorage is shared across tabs and survives page refreshes.  It is
 * cleared only on explicit logout via clearSessionPassword().
 *
 * @param {string} password  The user's plaintext password.
 */
