...
// ─── Session storage key (obfuscated, not secret) ────────────────────────────
const _SK = 'hfl_sp';

// Restore from sessionStorage on module load (survives page refreshes within
// the same browser tab; cleared automatically when the tab is closed).
let _sessionPassword = (
  typeof window !== 'undefined'
    ? (sessionStorage.getItem(_SK) ?? null)
    : null
);

/**
 * Store the user's password in memory and in sessionStorage so that
 * encrypt/decrypt helpers can derive per-file keys without the caller
 * passing the password every time.  Called immediately after a successful
 * login, before the password is discarded from the login form state.
 *
 * sessionStorage persists across same-tab page refreshes but is cleared
 * when the tab (or browser window) is closed — no persistent disk trace.
 *
 * @param {string} password  The user's plaintext password.
 */
export function setSessionPassword(password) {
  ...
  if (typeof window !== 'undefined') {
    sessionStorage.setItem(_SK, password);
  }
}

/**
 * Clear the in-memory password, the sessionStorage entry, and the
 * derived-key cache.  Must be called on logout so no key material lingers
...
/**
 * Return whether a session password is currently held in memory or sessionStorage.
 * Reads sessionStorage on every call so it stays accurate even if the module
 * was re-evaluated by Next.js (e.g. after navigation or hot-reload).
 */
export function hasSessionPassword() {
  if (_sessionPassword !== null) return true;
  // Re-hydrate from sessionStorage in case the module was re-evaluated
  if (typeof window !== 'undefined') {
...
/** Internal helper: ensure _sessionPassword is loaded (re-hydrates from sessionStorage if needed). */
...
export async function encryptWithSession(data) {
  return encryptData(_requirePassword(), data);
}
...
export async function decryptWithSession(payload) {
  return decryptData(_requirePassword(), payload);
}
