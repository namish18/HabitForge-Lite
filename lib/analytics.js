...
export function calculateWeeklyHabitConsistency(allLogs, tasks, subcategories, categories) {
  ...
  // Track habit completion per task
  const habitMap = {};
  const dailyCompletionData = [];

  // Initialize habitMap with all tasks
  tasks.forEach((task) => {
    const sub = subcategories.find((s) => s.id === task.subcategoryId);
    const cat = categories.find((c) => c.id === sub?.categoryId);
    habitMap[task.id] = {
      ...
      totalLogged: 0,
      completionRate: 0,
      weekActivity: weekDates.map((date) => ({ date, completed: false })),
    };
  });

  // Fill in actual completion data
  weekDates.forEach((date) => {
    const dayLogs = allLogs[date] || [];
    const dayCompletionMap = {};

    dayLogs.forEach((log) => {
      if (habitMap[log.taskId]) {
        dayCompletionMap[log.taskId] = log.completed;
        habitMap[log.taskId].totalLogged += 1;
      }
    });

    // Update activity for tasks
    tasks.forEach((task) => {
      const dateIndex = habitMap[task.id].weekActivity.findIndex((a) => a.date === date);
      if (dateIndex >= 0) {
        habitMap[task.id].weekActivity[dateIndex].completed = dayCompletionMap[task.id] || false;
      }

      if (dayCompletionMap[task.id]) {
        habitMap[task.id].completedDays += 1;
      } else if (dayCompletionMap[task.id] === false) {
        habitMap[task.id].missedDays += 1;
      }
    });
  });

  // Calculate completion rates and sort
  const habitStats = Object.values(habitMap).map((habit) => ({
    ...habit,
    completionRate: habit.totalLogged > 0 ? Math.round((habit.completedDays / habit.totalLogged) * 100) : 0,
  }));

  // Overall weekly stats
  const totalCompleted = habitStats.reduce((sum, h) => sum + h.completedDays, 0);
  const totalLogged = habitStats.reduce((sum, h) => sum + h.totalLogged, 0);
  const overallCompletionRate = totalLogged > 0 ? Math.round((totalCompleted / totalLogged) * 100) : 0;
  const habitsTrackedThisWeek = habitStats.filter((h) => h.totalLogged > 0).length;

  return {
    weeklyStats: {
      totalCompleted,
      totalLogged,
      totalMissed: totalLogged - totalCompleted,
      overallCompletionRate,
      habitsTrackedThisWeek,
      weekDates,
      weekLabels: weekDates.map((d) => format(parseISO(d), 'EEE')),
    },
    habits: habitStats.sort((a, b) => b.completionRate - a.completionRate),
  };
}
